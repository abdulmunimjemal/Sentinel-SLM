...
def prepare_rail_b(input_path, output_path, prototype=False, prototype_size=10000, seed=42):
    ...
    all_labels = [label for sublist in has_violations["labels"] for label in sublist if label != 0]
    ...
    def get_rarest_label(labels):
        valid_labels = [lbl for lbl in labels if lbl != 0 and lbl <= 7]  # 1-7 (PromptAttack=8 excluded)
        if not valid_labels:
            return -1
        # Sort by count (ascending), return the one with lowest count
        return sorted(valid_labels, key=lambda lbl: label_counts[lbl])[0]
...
def main():
    ...
    parser.add_argument(
        "--input", type=str, default="data/processed/final_augmented_dataset_enriched.parquet"
    )
    ...
    prepare_rail_b(
        input_path=args.input, output_path=args.output, prototype=is_prototype, prototype_size=size
    )
